...
vec4 grid(vec3 fragPos3D, float scale, bool drawAxis) {
    ...
    // z axis
    if(mod(fragPos3D.x, 2) > -0.1 * minimumx && mod(fragPos3D.x, 2) < 0.1 * minimumx)
        color.xyz = vec3(0.2, 0.2, 0.2);
    if(mod(fragPos3D.z, 2) > -0.1 * minimumz && mod(fragPos3D.z, 2) < 0.1 * minimumz)
        color.xyz = vec3(0.2, 0.2, 0.2);
    // x axis
